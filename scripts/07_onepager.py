...
body { font: 11pt/1.10 "Segoe UI", -apple-system, Helvetica, Arial, sans-serif;
...
h2 { font-size: 11pt; text-transform: uppercase; letter-spacing: 0.5pt;
  color: #37567f; margin: 1.8mm 0 0.7mm; }
p { margin: 0 0 1.0mm; }
...
li { margin: 0 0 0.5mm; }
b { font-weight: 600; }
img { width: 54%; height: auto; display: block; margin: 0.5mm auto 0.5mm; }
...
with a dated iteration history, never inline strings; every call logs the model
id, prompt hash and input hash, so any sentence traces to the exact prompt and
news behind it. Built with Claude Code. MediaWiki API, yfinance, hmmlearn,
...
<p><i>The detector underneath:</i> stressed days move <b>{oos['ratio']:.2f}&times;</b> as much as calm
days, in {sg.get('n_folds_ratio_above_1')} of {sg.get('n_folds_with_both_states')} folds independently (p&nbsp;=&nbsp;{sg.get('sign_test_p', 0):.5f}).</p>

<h2>Impact &amp; value</h2>
<p><b>Who needs it.</b> A risk team asked why a stress signal fired that week. A
PM handing over a book they did not build. A researcher whose changepoint model
produces dates nobody can interpret. All three rely on memory today.</p>
<p><b>What changes.</b> The interpretation becomes a written record with dated
sources, produced the same way every time, carrying a number for how far to
...
from ordinary days &mdash; so this buys an auditable record of <i>why</i>, not a
trading signal.</p>

<h2>Reflections</h2>
<p><b>The test that failed is the one I would keep</b> &mdash; but it was never
going to work. With only {pb.get('transitions', {}).get('n', 20)} real transitions it had a {100 * pw:.0f}% chance of
spotting a difference even if one was there. More ordinary dates would not have
helped; only more transitions would. <b>Next:</b> test only the days volatility <i>starts</i>, not every change &mdash;
that roughly triples the number that count. One gap stays open: the cut-off is
midnight UTC, and Wikipedia reports the US close that same evening.</p>
